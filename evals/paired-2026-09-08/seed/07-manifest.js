...
const work=figma.root.children.find(p=>p.name==='Work');await figma.setCurrentPageAsync(work);
...
return {raw,normalized:norm(raw),counts:{components:starter.children.flatMap(n=>n.children||[]).filter(n=>n.type==='COMPONENT').length,variables:variables.length,styles:styles.length,workChildren:work.children.length}};
